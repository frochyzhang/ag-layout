...
func Register_Greeter_GRPCServer(srv GreeterGRPCServer) server.Option {
...
func newGreeterServiceInfo(hasStreaming bool, keepStreamingMethods bool, keepNonStreamingMethods bool) *serviceinfo.ServiceInfo {
	serviceName := "Greeter"
	handlerType := (*GreeterGRPCServer)(nil)
...
func createGreeterHandler(ctx context.Context, handler interface{}, arg, result interface{}) error {
	...
	case *streaming.Args:
		...
		resp, err := handler.(GreeterGRPCServer).CreateGreeter(ctx, req)
		if err != nil {
			return err
		}
		return st.SendMsg(resp)
	case *CreateGreeterHelloRequestArgs:
		success, err := handler.(GreeterGRPCServer).CreateGreeter(ctx, s.Req)
		if err != nil {
			return err
		}
...
func putGreeterHandler(ctx context.Context, handler interface{}, arg, result interface{}) error {
	...
	case *streaming.Args:
		...
		resp, err := handler.(GreeterGRPCServer).PutGreeter(ctx, req)
		if err != nil {
			return err
		}
		return st.SendMsg(resp)
	case *PutGreeterHelloRequestArgs:
		success, err := handler.(GreeterGRPCServer).PutGreeter(ctx, s.Req)
		if err != nil {
			return err
		}
